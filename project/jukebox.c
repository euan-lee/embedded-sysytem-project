				...
				else 
				{
					while ( songPtr <= strlen(song[0][1]) ) //while of Parse.
					{
						...
						else if (parseType == PARSE_TYPE_NOTE)
						{
							...
							{
								if (song[0][1][songPtr] == '+')		//높은 옥타브...
								{
									//printf ("HighOct!\r\n");
									newNote+=12;
									songPtr++;
								}
								else if (song[0][1][songPtr] == '-')	//낮은 옥타브...song_no
								{
									newNote-=12;
									songPtr++;
								}
						...
						else if (parseType == PARSE_TYPE_MAJOR)
						{
							...
							{
								//printf ("%c!\r\n",song_note[songPtr]);
								if ( song[0][1][songPtr] >= 'A' && song[0][1][songPtr] <= 'G' )
								{
									permaMajor[song[0][1][songPtr] - 'A'] = majorMinor;
									songPtr++;
								}
								else
								{
